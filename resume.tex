...
\begin{itemize}[leftmargin=\myMargin]
    ...
    \item Worked in the \textbf{Quant Research Data Team} of Optiver Delta1
    \item Added functionality to \textbf{create TCP/IP filters} from session configuration files for packet parsers and \textbf{optimised} them for performance.
    \item Added functionality to \textbf{convert timestamps} across timezones, accounting for Daylight Saving Time changes 
    \item \textbf{Replaced a saturated time-series Postgres database} with a OneTick database, and \textbf{optimised its schema} based on past SQL queries to \textbf{minimise query time}.
\end{itemize}
\vspace{3pt}
...
\small
 \begin{itemize}[ leftmargin=\myMargin]
    \setlength \itemsep{-0.2em}
  \item \textbf{Rated 1806} \textcolor{blue}{Expert} on Codeforces
...
\item \textbf{AIR 3} in Shaastra CP Potpourri (Mixed-bag coding contest) \\ \small {\textit{[Shaastra is Asia's largest student-run Techfest]}} 
    \item { \textbf{Global Rank 9} in CodeChef Starters 96} $\vert$ { \textbf{Global Rank 231} in Codeforces Round 881} $\vert$ \textbf{Global Rank 373} in Google Farewell Round $\vert$ \textbf{Rank 2} in Republic Day Contest '24 and \textbf{Rank 3} in Endgame Contest organised by Programming Club, IITM
  \end{itemize}
...
\begin{itemize}[ leftmargin=\myMargin]
    ...
    \noindent
     $\dagger$ - Graduate Course \\ $*$ - Ongoing
\end{itemize}
\end{multicols}
...
\vspace{-0.3cm}
 \begin{itemize}[leftmargin=\myMargin]
    \setlength \itemsep{-0.2em}
  \item \textbf{Languages}: C++, C, HDL (Verilog), OCaml, Python, Java, Prolog, Flex, Yacc, SQL, x86, MIPS and 8085 ASM, \LaTeX, HTML \& CSS, R
...
\item Secured \textbf{AIR 5} in JEE Mains '21 out of 1 million students  %\hfill {[20XX]}%
\item Secured \textbf{AIR 161} in JEE Advanced '21\hfill
...
\item Awarded KVPY Fellowship '20 with \textbf{AIR 338}
\item Winner of Mimamsa '22 at IISER Pune $\vert$ 4$^{th}$ place in Chemenigma '22 at IISC Bangalore $\vert$ Won Silver Medal in Homi Bhabha Science Competition (conducted in Maharashtra) \hfill \textbf{AIR} $\leftarrow$ All India Rank
\end{itemize} 
...
\begin{itemize}[leftmargin=\myMargin]
    \setlength \itemsep{-0.2em}
    \item Implemented a CPU with \textbf{Register file} and \textbf{ALU} supporting Arithmetic 
    and Logical instructions on both 8-bit integers and  12-bit floats
